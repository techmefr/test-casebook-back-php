...
| Slim | `slim/slim` | No bundled ORM/auth/validation — everything (auth middleware, validators, persistence) is hand-rolled; use `$app->handle($request)` directly for in-process testing (no separate test-HTTP-client package needed). No global fake-timer mechanism exists in PHP — scheduling tests use relative real dates unless the project wires its own swappable clock. See `docs/testing-guide/slim.md` |
| Mezzio | `mezzio/mezzio` | No bundled ORM/auth/validation, same as Slim; build/test a `Laminas\Stratigility\MiddlewarePipe` directly (no container needed for a test double). `RouteResult::isMethodFailure()` is the only way to distinguish 404 from 405 — Mezzio's `DispatchMiddleware` treats both as one undifferentiated failure unless you check it explicitly. See `docs/testing-guide/mezzio.md` |
| CodeIgniter 4 | `codeigniter4/framework` | No bundled auth/validation for a hand-rolled scenario (use a Filter, `CodeIgniter\Filters\FilterInterface`); test via `CodeIgniter\Test\FeatureTestTrait`. `ext-intl` is a hard install requirement, not optional. The router has **no 405 concept at all** (a wrong verb on a real path is a plain 404). An unmatched route throws an uncaught `PageNotFoundException` in feature tests unless `$routes->set404Override(...)` is registered. See `docs/testing-guide/codeigniter4.md` |
...
1. Detect which framework is present via `composer.json` (`laravel/framework`, `symfony/framework-bundle`, `slim/slim`, `mezzio/mezzio`, `codeigniter4/framework`, or a different PHP framework). The core principles (plan first, isolate, permission matrix, review gate) apply regardless — the specific commands and idioms below are written Laravel-first with Symfony/Slim/Mezzio/CodeIgniter 4 call-outs; for a different framework, adapt to its own routing/DI/testing conventions.
