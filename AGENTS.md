...
1. Detect which framework is present via `composer.json` (`laravel/framework`, `symfony/framework-bundle`, `slim/slim`, `mezzio/mezzio`, or a different PHP framework). The core principles (plan first, isolate, permission matrix, review gate) apply regardless — the specific commands and idioms below are written Laravel-first with Symfony/Slim/Mezzio call-outs; for a different framework, adapt to its own routing/DI/testing conventions.
